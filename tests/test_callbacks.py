...
def test_update_leaderboard_callback():
    fig = update_leaderboard("Defense", [], True, True, False, True, True, False, [])
    assert isinstance(fig, go.Figure)


def test_update_details_callback(mocker):
    # Mock ensure_pokemon_image and has_shiny_artwork
...
def test_update_selector_options_callback():
...
def test_update_details_callback_high_stat(mocker):
    ...
    mocker.patch("app.ensure_pokemon_image", return_value="assets/images/6.png")
    mocker.patch("app.has_shiny_artwork", return_value=True)
...
def test_update_scatter_callback():
    fig = update_scatter(
